...
$query = "SELECT ifnull(SUM(amount), 0) AS total_amount FROM transactions WHERE status = 'pending' AND YEAR(created_at) = ? AND MONTH(created_at) = ?";
$stmt = $pdo->prepare($query);
$stmt->execute([$year, $month]);
$totalAmountCurrentMonth = $stmt->fetchColumn();

// Get the total amount for completed transactions in the previous month
$query = "SELECT ifnull(SUM(amount), 0) AS total_amount FROM transactions WHERE status = 'pending' AND YEAR(created_at) = YEAR(STR_TO_DATE(CONCAT(?, '-', ?, '-01'), '%Y-%m-%d')) - INTERVAL 1 MONTH AND MONTH(created_at) = MONTH(STR_TO_DATE(CONCAT(?, '-', ?, '-01'), '%Y-%m-%d')) - INTERVAL 1 MONTH";
$stmt = $pdo->prepare($query);
